...
describe("findStartPoint", () => {
  it("missing start point", () => {
    const missingStartCharacterData: string[][] = invalidMockData["invalid_example_1"].characters;
    const startPoint = findStartPoint(missingStartCharacterData);
    expect(startPoint).toEqual({ x: -1, y: -1 });
  });
  it("has start point", () => {
    const startCharacterData: string[][] = validMockData["valid_example_1"].characters;
